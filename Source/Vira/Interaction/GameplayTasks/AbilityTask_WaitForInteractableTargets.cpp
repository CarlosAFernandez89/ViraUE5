...
void UAbilityTask_WaitForInteractableTargets::AimWithDirection(const AActor* InSourceActor, FCollisionQueryParams Params, const FVector& TraceStart, float MaxRange, FVector& OutTraceEnd, bool bIgnorePitch) const
{
	if (!Ability)
	{
		return;
	...
	FHitResult HitResult;
	LineTrace(HitResult, InSourceActor->GetWorld(), TraceStart, TraceEnd, TraceProfile.Name, Params);

	const bool bUseTraceResult = HitResult.bBlockingHit && (FVector::DistSquared(TraceStart, HitResult.Location) <= (MaxRange * MaxRange));

	const FVector AdjustedEnd = (bUseTraceResult) ? HitResult.Location : TraceEnd;

	FVector AdjustedAimDir = (AdjustedEnd - TraceStart).GetSafeNormal();
	
	if (!bTraceAffectsAimPitch && bUseTraceResult)
	{
		FVector OriginalAimDir = InSourceActor->GetActorForwardVector(); // Use the given AimDirection directly
